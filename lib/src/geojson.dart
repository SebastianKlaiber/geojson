...
import 'package:geojson/src/deserializers.dart';
import 'package:geojson/src/exceptions.dart';
import 'package:geojson/src/models.dart';
...
class GeoJson {
  ...
  /// Parse the data
  Future<void> parse(String data, {String? nameProperty, bool verbose = false, bool disableStream = false}) =>
      _parse(data, nameProperty: nameProperty, verbose: verbose, disableStream: disableStream);
  ...
  Future<void> parseInMainThread(
    ...
    final feats = StreamController<GeoJsonFeature<dynamic>?>();
    final sub = feats.stream.listen((f) {
      if (verbose) {
    ...
    _processFeatures(dataToProcess: dataToProcess, sink: feats.sink);
  ...
  Future<List<GeoJsonPoint>> geofenceDistance({
    ...
    final dataToProcess =
        _GeoFenceDistanceToProcess(points: points, point: point, distance: distance, verbose: verbose);
    unawaited(iso.run(<dynamic>[dataToProcess]));
    await finished.future;
    return foundPoints;
  }
